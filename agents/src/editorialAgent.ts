...
export async function runEditorialAgent(input: JobInput, update: (state: JobState['status'], progress?: Project['progress'], activityMessage?: string) => void | Promise<void>): Promise<CompleteProjectReport> {
  ...
  const assets = new FunctionTool({ name: 'generate_editorial_assets', description: 'Have Gemini direct one analysis-grounded soundtrack containing the required music/sound moments, generate it with Lyria or the Treblo circuit-breaker fallback, listen to and index its usable ranges, and generate paired Gemini visual assets. This does not invent a continuous background bed.', parameters: reasonSchema, execute: async (_reason, toolContext) => {
    ...
    try {
      ...
      return { ok: true, needed: soundtrack.needed, generatedCues: soundtrack.cues.length, rationale: soundtrack.rationale };
    } catch (error) {
...
Current durable state: ${JSON.stringify(progressSummary(progress))}`,
    tools: [diagnose, creatorEvidence, plan, assets, render, review],
  });

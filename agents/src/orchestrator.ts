import type { Project } from '@dailies/shared'; import { analyzeVideo } from './analysisAgent.js'; import { createEditPlan } from './editingAgent.js'; import { generateScore } from './scoreAgent.js'; import { queryRetention, recommendationFromRows } from './retentionAgent.js'; import { renderFinalCut } from './renderAgent.js';
...
export async function runWorkflow(input: JobInput, update: (state: JobState['status'], progress?: Project['progress']) => void | Promise<void>) {
  ...
  checkpoint = { ...checkpoint, soundtrack }; await update(checkpoint.recommendation || !input.creatorHistoryEnabled ? 'editing' : 'querying_insights', checkpoint);
  const recommendation = checkpoint.recommendation || await resolveCreatorRecommendation(input.ownerId, input.durationSeconds, input.creatorHistoryEnabled === true);

...
export function buildFilterComplex(plan: EditPlan, cues: MusicCue[] = [], editorialCues: EditorialAudioCue[] = []) {
  ...
  effects.forEach(({ cue, mapped }, index) => {
    const start = mapped!.start; const duration = Math.min(1.8, Math.max(.7, mapped!.end - mapped!.start)); const end = Math.min(outputCursor, start + duration);
    ...
    filters.push(`[${videoLabel}][${overlay}]overlay=x=W-w-48:y=48:eof_action=pass:enable='between(t,${number(start)},${number(end)})'[${next}]`); videoLabel = next;
  });
  filters.push(`[${videoLabel}]null[vout]`);
  let dialogueLabel = 'dialogue';
  effects.forEach(({ cue, mapped }, index) => { const duckDuration = cue.type === 'sting' ? .95 : .55; const next = `dialogueduck${index}`; filters.push(`[${dialogueLabel}]volume=0.32:enable='between(t,${number(mapped!.start)},${number(Math.min(outputCursor, mapped!.start + duckDuration))})'[${next}]`); dialogueLabel = next; });
  ...
  if (additions.length) filters.push(`[${dialogueLabel}]${additions.join('')}amix=inputs=${additions.length + 1}:duration=first:dropout_transition=0:normalize=0,loudnorm=I=${number(plan.audioCleanup.targetLufs)}:LRA=11:TP=-1.5,aresample=48000,aformat=sample_rates=48000:channel_layouts=stereo[aout]`);

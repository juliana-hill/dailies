...
- `agents/`: Node.js/TypeScript Express service running Google's official TypeScript ADK. A Gemini `LlmAgent` owns the editorial objective and invokes source diagnosis, optional ClickHouse creator evidence, edit planning, Lyria/Gemini asset generation, FFmpeg/Cloud rendering, rendered-draft review, and targeted revision tools. It may render and inspect up to three drafts and cannot mark a project complete until the rendered-video reviewer passes it. Pipeline jobs, draft iterations, reviews, leases, checkpoints, and an append-only event ledger are persisted in Firestore.
...
Each project has one durable job document in `dailies_pipeline_jobs`. The document stores the latest diagnosis, creator evidence, generated assets, edit plan, draft render, rendered-draft review, iteration number, active worker lease, retry timing, and final report. ADK reconstructs its working context from those checkpoints after recovery. Its `events` subcollection is an append-only execution record covering submission, lease ownership, checkpoints, automatic retry scheduling, draft/review progress, completion, and terminal failure. Workers renew short leases while running; another instance reclaims an expired lease after a crash or deployment. Every revision receives a distinct deterministic render ID so a previous cloud draft cannot be mistaken for the revised cut. Transient failures use bounded exponential retry; invalid inputs and missing configuration fail immediately for operator action.
